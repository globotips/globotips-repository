...
7. Sign in as the Tampa hotel. For an existing employee, click **Start Stripe**. Adding a new employee starts Express onboarding immediately.
8. Complete Express onboarding with Stripe test data. The QR is **live** only after `charges_enabled` and `payouts_enabled` are true.
